...
void normal(int c) {
	...
	case 'b':
		...
		for (n = DEFAULT1(Prenum); n > 0; n--)
		{
			...
			if ((Curschar->linep->prev == Filetop->linep) && (Curschar->index == 0))
			{
				CLEAROP;
				beep();
				break;
 ...
 */
static void tabinout(int shift_type, int num)
{
	LPtr *p;
 ...
 */
static void dodelete(bool_t redraw, bool_t setup_for_undo, bool_t try_to_yank)
{
	LPtr top, bot;
	int nlines;
	int n;
...
static void startinsert(int startln)
{
	*Insstart = *Curschar;
	if (startln)
	{
...
void AppendNumberToUndobuff(int n)
{
	char buf[32];
...
void AppendPositionToUndobuff(int column, int row)
{
	if (UndoInProgress)
		return;
...
void AppendToUndoUndobuff(char *s)
{
	if (UndoInProgress)
		return;
...
void AppendNumberToUndoUndobuff(int n)
{
	char buf[32];
...
void AppendPositionToUndoUndobuff(int column, int row)
{
	if (UndoInProgress)
		return;
...
static bool_t dojoin(bool_t leading_space, bool_t strip_leading_spaces)
{
